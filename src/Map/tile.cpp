...
void tile::display(){
	...
	glBegin(GL_QUADS);
		for (int i = 0; i < 5; ++i){
			glNormal3f(cos(to_rad(d)), 0, sin(to_rad(d)));
			glVertex3f(p[i].x,p[i].y,p[i].z);
			glVertex3f(p[i].x,p[i].y-0.5,p[i].z);
			glVertex3f(p[i+1].x,p[i+1].y-0.5,p[i+1].z);
			glVertex3f(p[i+1].x,p[i+1].y,p[i+1].z);
			d -= 60;
		}
		glNormal3f(cos(to_rad(d)), 0, sin(to_rad(d)));
		glVertex3f(p[5].x,p[5].y,p[5].z);
		glVertex3f(p[5].x,p[5].y-0.5,p[5].z);
		glVertex3f(p[0].x,p[0].y-0.5,p[0].z);
